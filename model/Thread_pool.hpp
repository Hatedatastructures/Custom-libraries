...
using namespace _implemented_internally;
using namespace _implemented_internally::structure_task;
using namespace _implemented_internally::structure_cohort;
using namespace _implemented_internally::structure_worker;
using namespace _implemented_internally::structure_scheduler;
/**
 * @enum pool_state
 * @brief 线程池状态枚举
 */
...
public: 
   ...
   */
  template <typename function, typename... Args>
  std::string submit_void(function &&func, Args &&...args)
  {
    if (!is_running())
    {
      throw std::runtime_error("Thread pool is not running");
...
public: 
  ...
  std::string get_performance_report() const
  {
    ...
    oss << "=== Thread Pool Performance Report ===\n";
    oss << "Pool Name: " << _config.pool_name                 << "\n";
    oss << "State: "     << static_cast<int>(_state.load())   << "\n";
    oss << "Uptime: "    << std::fixed                        << std::setprecision(2)
        << _statistics.calculate_uptime()                     << " seconds\n";

    oss << "\n--- Task Statistics ---\n";
    oss << "Total Submitted: " << _statistics.total_tasks_submitted.load() << "\n";
    oss << "Total Completed: " << _statistics.total_tasks_completed.load() << "\n";
    oss << "Total Failed: "    << _statistics.total_tasks_failed.load()    << "\n";
    oss << "Total Timeout: "   << _statistics.total_tasks_timeout.load()   << "\n";
    oss << "Total Cancelled: " << _statistics.total_tasks_cancelled.load() << "\n";
    ...
    oss << "Current Throughput: "    << std::fixed << std::setprecision(2)
        << _statistics.current_throughput.load()   << " tasks/sec\n";
    oss << "Peak Throughput: "       << std::fixed << std::setprecision(2)
        << _statistics.peak_throughput.load()      << " tasks/sec\n";
    oss << "Average Task Duration: " << std::fixed << std::setprecision(2)
        << _statistics.average_task_duration.load()<< " ms\n";
    oss << "Average Wait Time: "     << std::fixed << std::setprecision(2)
        << _statistics.average_wait_time.load()    << " ms\n";

    oss << "\n--- Thread Statistics ---\n";
    oss << "Current Threads: " << _statistics.current_thread_count.load() << "\n";
    oss << "Active Threads: "  << _statistics.active_thread_count.load()  << "\n";
    oss << "Idle Threads: "    << _statistics.idle_thread_count.load()    << "\n";
    oss << "Peak Threads: "    << _statistics.peak_thread_count.load()    << "\n";

    oss << "\n--- Queue Statistics ---\n";
    oss << "Current Queue Size: " << _statistics.current_queue_size.load()  << "\n";
    oss << "Peak Queue Size: "    << _statistics.peak_queue_size.load()     << "\n";
    oss << "Queue Utilization: "  << std::fixed                             << std::setprecision(2)
        << (get_queue_utilization() * 100)                                  << "%\n";

    oss << "\n--- Scaling Statistics ---\n";
    oss << "Scale Up Operations: "   << _statistics.total_scale_up_operations.load()   << "\n";
    oss << "Scale Down Operations: " << _statistics.total_scale_down_operations.load() << "\n";
  ...
  std::string get_health_status() const
  {
    ...
    oss << "=== Thread Pool Health Status ===\n";

    // 整体状态
    bool overall_healthy = health_check();
    oss << "Overall Status: " << (overall_healthy ? "HEALTHY" : "UNHEALTHY") << "\n";

    // 详细检查
    oss << "\n--- Component Status ---\n";
    oss << "Pool State: "        << static_cast<int>(_state.load())                           << "\n";
    oss << "Scheduler Running: " << (_scheduler && _scheduler->is_running() ? "YES" : "NO")   << "\n";
    oss << "Queue Open: "        << (_task_queue && !_task_queue->closed() ? "YES" : "NO") << "\n";

    oss << "\n--- Resource Status ---\n";
    auto thread_count = get_thread_count();
    oss << "Thread Count: "      << thread_count        << " (" << _config.min_threads
        << "-"                   << _config.max_threads << ")\n";
    oss << "Queue Size: "        << get_queue_size()    << "/"  << get_queue_capacity() << "\n";
    oss << "Queue Utilization: " << std::fixed << std::setprecision(1)
        << (get_queue_utilization() * 100)              << "%\n";
