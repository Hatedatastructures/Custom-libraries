...
class thread_pool
{
  ...
  static constexpr std::chrono::milliseconds _dormancy  = std::chrono::milliseconds(50);  //线程休眠时间
  static constexpr std::chrono::milliseconds _frequency = std::chrono::milliseconds(500); //调整频率
  static constexpr std::chrono::milliseconds _timeout   = std::chrono::milliseconds(200); //线程超时时间
  ...
  producers_consumers_queue<func_t> _tasks; //任务队列
  ...
  void shrink_capacity(uint64_t counter)
  {
    ...
    {
      if (_closure_threads.load() > _min_threads && (now - it->_last_active) >= _timeout)
      {
