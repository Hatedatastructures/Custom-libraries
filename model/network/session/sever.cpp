...
private:
  void _do_accept()
  {
    auto function = [this, self = shared_from_this()](boost::system::error_code ec, boost::asio::ip::tcp::socket socket)
    {
      if(!ec)
      {
        ...
        if(!ep_ec)
          ...
          _manager.get_session_count());

        auto pair = _manager.create_server_session(std::move(socket));
        auto new_session = pair.second;
        if(new_session)
        {
          auto auto_reception_processing = [](std::shared_ptr<session<>> sess, std::string_view data)
          { sess->async_send_bytes(std::string(data), nullptr); };
