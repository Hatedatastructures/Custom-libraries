...
namespace tree_container
{
    ...
    */
    ...
    private:
        ...
        public:
            // 节点类
            binary_search_tree_type_node *left_;
            binary_search_tree_type_node *right_;
            binary_search_tree_type data_;
            explicit binary_search_tree_type_node(const binary_search_tree_type &binary_search_tree_type_data = binary_search_tree_type())
                    : left_(nullptr), right_(nullptr), data_(binary_search_tree_type_data)
            {
                ;
            }
        ...
        void interior_middle_order_traversal(container_node *root_subtree_node)
        {
            ...
            while (root_subtree_node != nullptr || !interior_stack.empty())
            {
                while (root_subtree_node != nullptr)
                {
                    ...
                    // 压栈
                    root_subtree_node = root_subtree_node->left_;
                }
                // 访问栈顶节点
                ...
                interior_stack.pop();
                std::cout << root_subtree_node->data_ << " ";
                // std::cout << &root_subtree_node->data_ << " ";
                // 检查地址是不是值拷贝
                // 转向右子树
                root_subtree_node = root_subtree_node->right_;
            }
        }
        ...
        void interior_pre_order_traversal(container_node *root_subtree_node)
        {
            ...
            while (!interior_stack.empty())
            {
                ...
                std::cout << reference_node->data_ << " ";
                if (reference_node->right_ != nullptr)
                {
                    interior_stack.push(reference_node->right_);
                }
                if (reference_node->left_ != nullptr)
                {
                    interior_stack.push(reference_node->left_);
        ...
        void clear() noexcept
        {
            ...
            resource_release_stack.push(root_);
            while (resource_release_stack.empty() == false)
            {
                ...
                if (pending_deletion_node->left_ != nullptr)
                {
                    resource_release_stack.push(pending_deletion_node->left_);
                }
                if (pending_deletion_node->right_ != nullptr)
                {
                    resource_release_stack.push(pending_deletion_node->right_);
                }
    ...
    public:
        ...
        explicit binary_tree(const binary_search_tree_type &bstt_node = binary_search_tree_type())
                : root_(nullptr)
        {
            root_ = new container_node(bstt_node);
        }
        binary_tree(binary_tree &&binary_search_tree_object) noexcept
                : root_(nullptr), function_policy(binary_search_tree_object.function_policy)
        {
            root_ = std::move(binary_search_tree_object.root_);
            binary_search_tree_object.root_ = nullptr;
        }
        ...
        {
            // 拷贝构造，时间复杂度为O(n)
            container_node *reference_node = binary_search_tree_object.root_;
            ...
            // 注意这里把本地root_类型传过去，是因为要对本地的root_进行操作，所以要传二级指针
            // 这里传引用也不行，这里的对象是动态变化的，所以传引用也不行
            // 如果是对全局的_ROOT进行操作，就传一级指针
            interior_stack.push(standard_con::pair<container_node *, container_node **>(reference_node, &root_));
            while (!interior_stack.empty())
            {
                auto pair_node = interior_stack.top();
                interior_stack.pop();
                *(pair_node.second) = new container_node(pair_node.first->data_);
                // container_node* _staic_temp_pair_second = *(pair_node.second);
                // if(pair_node.first->left_!= nullptr)
                // { //远古版本
                //     interior_stack.push(MY_Template::practicality::pair<container_node*,container_node**>
                //     (pair_node.first->left_,&_staic_temp_pair_second->left_));
                // }
                // if(pair_node.first->right_!= nullptr)
                // {
                //     interior_stack.push(MY_Template::practicality::pair<container_node*,container_node**>
                //     (pair_node.first->right_,&_staic_temp_pair_second->right_));
                // }
                // 移除临时变量，直接使用指针解引用
                if (pair_node.first->right_ != nullptr)
                {
                    interior_stack.push(standard_con::pair<container_node *, container_node **>(pair_node.first->right_, &((*pair_node.second)->right_)));
                }
                if (pair_node.first->left_ != nullptr)
                {
                    interior_stack.push(standard_con::pair<container_node *, container_node **>(pair_node.first->left_, &((*pair_node.second)->left_)));
                }
            }
        }
        void middle_order_traversal()
        {
            interior_middle_order_traversal(root_);
        }
        void pre_order_traversal()
        {
            interior_pre_order_traversal(root_);
        }
        bool push(const binary_search_tree_type &binary_search_tree_type_data)
        {
            if (root_ == nullptr)
            {
                root_ = new container_node(binary_search_tree_type_data);
                return true;
            }
            else
            {
                container_node *reference_node = root_;
                container_node *subtree_node = nullptr;
                while (reference_node != nullptr)
                {
                    subtree_node = reference_node;
                    if (!function_policy(binary_search_tree_type_data, reference_node->data_) &&
                            !function_policy(reference_node->data_, binary_search_tree_type_data))
                    {
                        // 改用仿函数特性，判断是否有重复元素,防止自定义类型没有重载==运算符
                        return false;
                    }
                    else if (function_policy(binary_search_tree_type_data, reference_node->data_))
                    {
                        reference_node = reference_node->left_;
                    }
                    else
                    {
                        reference_node = reference_node->right_;
                    }
                }
                // 新开节点链接
                auto *new_element_node = new container_node(binary_search_tree_type_data);
                // 链接节点
                if (function_policy(binary_search_tree_type_data, subtree_node->data_))
                {
                    subtree_node->left_ = new_element_node;
                }
                else
                {
                    subtree_node->right_ = new_element_node;
                }
                return true;
            }
        }
        binary_tree &pop(const binary_search_tree_type &binary_search_tree_type_data)
        {
            ...
            while (reference_node != nullptr)
            {
                if (binary_search_tree_type_data == reference_node->data_)
                {
                    // 找到节点
                    if (reference_node->left_ == nullptr)
                    {
                        ...
                        {
                            // 当前节点是根节点，直接更新 root_
                            root_ = reference_node->right_;
                        }
                        else
                        {
                            if (subtree_node->left_ == reference_node)
                            {
                                // 根节点
                                subtree_node->left_ = reference_node->right_;
                            }
                            else
                            {
                                // 非根节点
                                subtree_node->right_ = reference_node->right_;
                            }
                        }
                    ...
                    else if (reference_node->right_ == nullptr)
                    {
                        if (subtree_node == nullptr)
                        {
                            // 防止当前节点是根节点，无法解引用，直接更新 root_
                            root_ = reference_node->left_;
                        }
                        else
                        {
                            if (subtree_node->left_ == reference_node)
                            {
                                subtree_node->left_ = reference_node->left_;
                            }
                            else
                            {
                                subtree_node->right_ = reference_node->left_;
                            }
                        }
                        delete reference_node;
                        reference_node = nullptr;
                    ...
                    {
                        // 左右子树都不为空，找右子树的最左节点
                        container_node *right_subtree_least_node = reference_node->right_;
                        container_node *subtree_parent_node = reference_node;
                        while (right_subtree_least_node->left_ != nullptr)
                        {
                            subtree_parent_node = right_subtree_least_node;
                            right_subtree_least_node = right_subtree_least_node->left_;
                        }
                        // 找到最左节点
                        standard_con::algorithm::swap(reference_node->data_, right_subtree_least_node->data_);
                        ...
                        if (subtree_parent_node == reference_node)
                        {
                            ...
                            subtree_parent_node->right_ = right_subtree_least_node->right_;
                            // 这俩交换指针指向位置就行，上面已经完成值的替换
                        }
                        else
                        {
                            // 情况2：说明要删除的数据的右子树的最左节点如果有数据，就把数据连接到右子树的最左节点的父亲节点的左子树指向最左子树的右子树
                            subtree_parent_node->left_ = right_subtree_least_node->right_;
                        }
        ...
        {
            uint64_t _size = 0;
            return interior_middle_order_traversal(root_, _size);
        }
        [[nodiscard]] uint64_t size() const
        {
            uint64_t node_number_counter = 0;
            return interior_middle_order_traversal(root_, node_number_counter);
        }
        container_node *find(const binary_search_tree_type &find_node)
        {
            ...
            while (reference_node != nullptr)
            {
                if (find_node == reference_node->data_)
                {
                    return reference_node;
                }
                else if (function_policy(find_node, reference_node->data_))
                {
                    reference_node = reference_node->left_;
                }
                else
        ...
        void insert(const binary_search_tree_type &existing_value, const binary_search_tree_type &new_value)
        {
            ...
            else
            {
                ...
                new_value_node->left_ = existing_value_node->right_;
                existing_value_node->right_ = new_value_node;
            }
        }
        binary_tree &operator=(const binary_tree &binary_search_tree_object)
        {
            ...
            if (this != &binary_search_tree_object)
            {
                ...
                standard_con::algorithm::swap(reference_node.root_, root_);
            }
            return *this;
        }
    ...
    */
    ...
    private:
        ...
        {
        public:
            avl_tree_node_pair data_;

            avl_tree_type_node *left_;
            avl_tree_type_node *right_;
            avl_tree_type_node *parent_;
            // 平衡因子
            int balance_factor_;
            explicit avl_tree_type_node(const avl_tree_type_k &avl_tt_k_data = avl_tree_type_k(), const avl_tree_type_v &avl_tt_v_data = avl_tree_type_v())
                    : data_(avl_tt_k_data, avl_tt_v_data), left_(nullptr), right_(nullptr), parent_(nullptr), balance_factor_(0)
            {
                ;
            }
            explicit avl_tree_type_node(const avl_tree_node_pair &pair_type_data)
                    : data_(pair_type_data), left_(nullptr), right_(nullptr), parent_(nullptr), balance_factor_(0)
            {
                ;
            }
        };
        ...
        public:
            ...
            using pointer = Ptr;
            using reference = Ref;
            iterator_node *node_iterator_ptr_;
            explicit avl_tree_iterator(iterator_node *iterator_ptr_data)
                    : node_iterator_ptr_(iterator_ptr_data) { ; }

            Ptr operator->() { return &(node_iterator_ptr_->data_); }

            Ref &operator*() { return node_iterator_ptr_->data_; }

            bool operator!=(const self &another_iterator) { return node_iterator_ptr_ != another_iterator.node_iterator_ptr_; }

            bool operator==(const self &another_iterator) { return node_iterator_ptr_ == another_iterator.node_iterator_ptr_; }
            self &operator++()
            {
                if (node_iterator_ptr_->right_ != nullptr)
                {
                    node_iterator_ptr_ = node_iterator_ptr_->right_;
                    while (node_iterator_ptr_->left_ != nullptr)
                    {
                        node_iterator_ptr_ = node_iterator_ptr_->left_;
                    }
                }
                else
                {
                    container_node *_iterator_self_node = node_iterator_ptr_;
                    while (_iterator_self_node->parent_ != nullptr && _iterator_self_node == _iterator_self_node->parent_->right_)
                    {
                        _iterator_self_node = _iterator_self_node->parent_;
                    }
                    node_iterator_ptr_ = _iterator_self_node->parent_;
                }
                return *this;
            }
            self operator++(int)
            ...
            self &operator--()
            {
                if (node_iterator_ptr_->left_ != nullptr)
                {
                    node_iterator_ptr_ = node_iterator_ptr_->left_;
                    while (node_iterator_ptr_->right_ != nullptr)
                    {
                        node_iterator_ptr_ = node_iterator_ptr_->right_;
                    }
                }
                ...
                {
                    container_node *_iterator_self_node = node_iterator_ptr_;
                    while (_iterator_self_node->parent_ != nullptr && _iterator_self_node == _iterator_self_node->parent_->left_)
                    {
                        _iterator_self_node = _iterator_self_node->parent_;
                    }
                    node_iterator_ptr_ = _iterator_self_node->parent_;
        ...
        public:
            ...
            bool operator!=(const self &another_iterator) { return it_ != another_iterator.it_; }
        ...
        void left_revolve(container_node *&subtree_node)
        {
            /*                                                                                                              左单旋情况：简化图
                    ...
                    {                                                                                            sub_right_left_node(nullptr)  sub_tree_right_right_node(30)
                            container_node* sub_tree_right_node = subtree_node->right_;                                  /############             分割线             ############/
                            subtree_node->right_ = sub_tree_right_node->left_;                                                          sub_tree_right_node(20)(当前平衡因子为0)
                            sub_tree_right_node->left_ = subtree_node;                                                                        /       \
                            //错误写法：未同步调整父亲节点和判断调整节点的左根节点是否为空，以及全部需要调整节点的父亲指针的指针的指向        subtree_node(10)    sub_tree_right_right_node(30)
                    }                                                                                                                 \
                    if(subtree_node == nullptr || subtree_node->right_ == nullptr)                                 sub_right_left_node(nullptr)
                    {
            ...
            */
            ...
            container_node *sub_tree_right_node = subtree_node->right_;
            // container_node* sub_right_left_node = sub_tree_right_node->left_;
            container_node *sub_right_left_node = (sub_tree_right_node->left_) ? sub_tree_right_node->left_ : nullptr;
            // 防止空指针解引用
            subtree_node->right_ = sub_right_left_node; // 因为调整后parent_node的位置是sub_right_left_node的位置
            if (sub_right_left_node)                    // 把parent_node的右根节点赋值给sub_right_left_node
            {
                sub_right_left_node->parent_ = subtree_node;
                // 如果sub_right_left_node(调整节点的左根节点)不等于空，还需要调整sub_right_left_node它的父亲节点
            }
            sub_tree_right_node->left_ = subtree_node;
            // 这里先保存一下parent_node的父亲地址，防止到下面else比较的时候丢失
            container_node *parent_node = subtree_node->parent_;
            subtree_node->parent_ = sub_tree_right_node;
            // 更新parent_node节点指向正确的地址

            if (root_ == subtree_node)
            {
                // 如果要调整的节点是根节点，直接把调整节点赋值给根节点，然后把调整节点的父亲节点置空
                root_ = sub_tree_right_node;
                sub_tree_right_node->parent_ = nullptr;
            }
            else
            {
                // 调整前parent_node是这个树的根现在是sub_tree_right_node是这个树的根
                if (parent_node->left_ == subtree_node)
                {
                    parent_node->left_ = sub_tree_right_node;
                }
                else
                {
                    parent_node->right_ = sub_tree_right_node;
                }
                sub_tree_right_node->parent_ = parent_node;
            }
            subtree_node->balance_factor_ = sub_tree_right_node->balance_factor_ = 0;
        }
        ...
        void right_revolve(container_node *&subtree_node)
        {
            ...
            container_node *sub_tree_left_node = subtree_node->left_;
            container_node *sub_left_right_node = (sub_tree_left_node->right_) ? sub_tree_left_node->right_ : nullptr;
            // 防止空指针解引用
            subtree_node->left_ = sub_left_right_node;
            if (sub_left_right_node)
            {
                sub_left_right_node->parent_ = subtree_node;
            }
            sub_tree_left_node->right_ = subtree_node;
            // 保存parent_temp_Node的父亲节点
            container_node *parent_node = subtree_node->parent_;
            subtree_node->parent_ = sub_tree_left_node;

            if (root_ == subtree_node)
            {
                root_ = sub_tree_left_node;
                sub_tree_left_node->parent_ = nullptr;
            }
            else
            {
                if (parent_node->left_ == subtree_node)
                {
                    parent_node->left_ = sub_tree_left_node;
                }
                else
                {
                    parent_node->right_ = sub_tree_left_node;
                }
                sub_tree_left_node->parent_ = parent_node;
            }
            subtree_node->balance_factor_ = sub_tree_left_node->balance_factor_ = 0;
        }
        void right_left_revolve(container_node *&subtree_node)
        {
            ...
            container_node *sub_tree_right_node = subtree_node->right_;
            container_node *sub_right_left_node = sub_tree_right_node->left_;
            int separate_balance_factor = sub_right_left_node->balance_factor_;

            right_revolve(subtree_node->right_);
            // 右旋
            left_revolve(subtree_node);
            // 左旋
            if (separate_balance_factor == -1)
            {
                subtree_node->balance_factor_ = 0;
                sub_tree_right_node->balance_factor_ = 1;
                sub_right_left_node->balance_factor_ = 0;
            }
            else if (separate_balance_factor == 1)
            {
                subtree_node->balance_factor_ = -1;
                sub_tree_right_node->balance_factor_ = 0;
                sub_right_left_node->balance_factor_ = 0;
            }
            else
            {
        ...
        void left_right_revolve(container_node *&subtree_node)
        {
            ...
            container_node *sub_tree_left_node = subtree_node->left_;
            container_node *sub_left_right_node = sub_tree_left_node->right_;
            int separate_balance_factor = sub_left_right_node->balance_factor_;

            left_revolve(subtree_node->left_);
            // 左旋
            ...
            {
                subtree_node->balance_factor_ = 0;
                sub_tree_left_node->balance_factor_ = 1;
                sub_left_right_node->balance_factor_ = 0;
            }
            else if (separate_balance_factor == 1)
            {
                subtree_node->balance_factor_ = -1;
                sub_tree_left_node->balance_factor_ = 0;
                sub_left_right_node->balance_factor_ = 0;
            }
            else
            {
        ...
        void clear() noexcept
        {
            ...
            else
            {
                standard_con::stack<container_node *> interior_stack;
                // 前序释放
                interior_stack.push(root_);
                while (!interior_stack.empty())
                {
                    container_node *delete_data_node = interior_stack.top();
                    interior_stack.pop();
                    if (delete_data_node->left_ != nullptr)
                    {
                        interior_stack.push(delete_data_node->left_);
                    }
                    if (delete_data_node->right_ != nullptr)
                    {
                        interior_stack.push(delete_data_node->right_);
                    }
    ...
    public:
        ...
        iterator begin()
        {
            container_node *start_position_node = root_;
            while (start_position_node != nullptr && start_position_node->left_ != nullptr)
            {
                start_position_node = start_position_node->left_;
            }
        ...
        {
            container_node *reverse_start_position = root_;
            while (reverse_start_position != nullptr && reverse_start_position->right_ != nullptr)
            {
                reverse_start_position = reverse_start_position->right_;
            }
        ...
        explicit balance_tree(const avl_tree_type_k &key_data, const avl_tree_type_v &val_data = avl_tree_type_v(),
                ...
                : root_(nullptr), function_policy(com_value)
        {
            root_ = new container_node(key_data, val_data);
        }
        explicit balance_tree(const avl_tree_node_pair &pair_type_data,
                                                                                container_imitate_function com_value = container_imitate_function())
                : root_(nullptr), function_policy(com_value)
        {
            root_ = new container_node(pair_type_data.first, pair_type_data.second);
        }
        balance_tree(const balance_tree &avl_tree_data)
                : root_(nullptr), function_policy(avl_tree_data.function_policy)
        {
            if (avl_tree_data.root_ == nullptr)
            {
                return;
            }
            ...
            // 创建根节点
            root_ = new container_node(avl_tree_data.root_->data_);
            root_->balance_factor_ = avl_tree_data.root_->balance_factor_;
            root_->parent_ = nullptr; // 根节点的父节点为nullptr

            // 初始化栈，将根节点的子节点压入（注意：这里父节点是 _ROOT，一级指针）
            if (avl_tree_data.root_->right_ != nullptr)
            {
                stack.push(standard_con::pair<container_node *, container_node *>(avl_tree_data.root_->right_, root_));
            }
            if (avl_tree_data.root_->left_ != nullptr)
            {
                stack.push(standard_con::pair<container_node *, container_node *>(avl_tree_data.root_->left_, root_));
            }
            ...
            while (!stack.empty())
            {
                ...
                // 创建新节点并复制数据
                auto *new_structure_node = new container_node(first_node->data_);
                new_structure_node->balance_factor_ = first_node->balance_factor_;

                // 设置父节点关系（注意：second_node 是一级指针）
                new_structure_node->parent_ = second_node;

                // 判断源节点在原树中是左子还是右子
                bool isleft_child = false;
                if (first_node->parent_ != nullptr)
                {
                    isleft_child = (first_node->parent_->left_ == first_node);
                }

                // 将新节点链接到父节点的正确位置（注意：直接使用 second_node）
                if (isleft_child)
                {
                    second_node->left_ = new_structure_node;
                }
                else
                {
                    second_node->right_ = new_structure_node;
                }

                // 处理子节点（注意：压栈时父节点是 new_structure_node，一级指针）
                if (first_node->right_ != nullptr)
                {
                    stack.push(standard_con::pair<container_node *, container_node *>(first_node->right_, new_structure_node));
                }
                if (first_node->left_ != nullptr)
                {
                    stack.push(standard_con::pair<container_node *, container_node *>(first_node->left_, new_structure_node));
                }
            }
        }
        balance_tree(balance_tree &&avl_tree_data) noexcept
                : root_(nullptr), function_policy(avl_tree_data.function_policy)
        {
            root_ = std::move(avl_tree_data.root_);
            avl_tree_data.root_ = nullptr;
        }
        ...
        balance_tree &operator=(const balance_tree avl_tree_data)
        {
            ...
            standard_con::algorithm::swap(root_, avl_tree_data.root_);
            return *this;
        }
        ...
        bool push(const avl_tree_type_k &key_data, const avl_tree_type_v &val_data = avl_tree_type_v())
        {
            ...
            else
            {
                ...
                while (reference_node)
                {
                    ...
                    if (!function_policy(key_data, reference_node->data_.first) && !function_policy(reference_node->data_.first, key_data))
                    {
                        return false;
                    }
                    else if (function_policy(key_data, reference_node->data_.first))
                    {
                        reference_node = reference_node->left_;
                    }
                    else
                ...
                if (function_policy(key_data, parent_node->data_.first))
                {
                    parent_node->left_ = reference_node;
                }
                else
                {
                    parent_node->right_ = reference_node;
                }
                reference_node->parent_ = parent_node;
                ...
                {
                    if (adjust_reference_parent_node->left_ == adjust_reference_node)
                    {
                        --adjust_reference_parent_node->balance_factor_;
                    }
                    else
                    {
                        ++adjust_reference_parent_node->balance_factor_;
                    ...
                    if (adjust_reference_parent_node->balance_factor_ == 0)
                    {
                        break;
                    }
                    else if (adjust_reference_parent_node->balance_factor_ == 1 || adjust_reference_parent_node->balance_factor_ == -1)
                    {
                        adjust_reference_node = adjust_reference_parent_node;
                        adjust_reference_parent_node = adjust_reference_parent_node->parent_;
                    }
                    else if (adjust_reference_parent_node->balance_factor_ == 2 || adjust_reference_parent_node->balance_factor_ == -2)
                    {
                        if (adjust_reference_parent_node->balance_factor_ == 2)
                        {
                            if (adjust_reference_node->balance_factor_ == 1)
                            {
        ...
        bool push(const avl_tree_node_pair &pair_type_data)
        {
            ...
            else
            {
                ...
                while (reference_node != nullptr)
                {
                    parent_node = reference_node;
                    // 找到first该在的节点
                    if (!function_policy(pair_type_data.first, reference_node->data_.first) && !function_policy(reference_node->data_.first, pair_type_data.first))
                    {
                        // 不允许重复插入
                        return false;
                    }
                    else if (function_policy(pair_type_data.first, reference_node->data_.first))
                    {
                        reference_node = reference_node->left_;
                    }
                    else
                ...
                if (function_policy(pair_type_data.first, parent_node->data_.first))
                {
                    parent_node->left_ = reference_node;
                    // 三叉链表，注意父亲节点指向
                }
                ...
                {
                    // 更新到根节点跳出
                    if (adjust_reference_node == adjust_reference_parent_node->right_)
                    {
                        ++adjust_reference_parent_node->balance_factor_;
                    }
                    else
                    ...
                    else if (adjust_reference_parent_node->balance_factor_ == 2 || adjust_reference_parent_node->balance_factor_ == -2)
                    {
                        ...
                        if (adjust_reference_parent_node->balance_factor_ == 2)
                        {
                            if (adjust_reference_node->balance_factor_ == 1)
                            {
                                // L，说明_ROOT_Temp_test是_ROOT_Temp_testparent_的左子节点，线形
                                left_revolve(adjust_reference_parent_node);
                            }
                            else
                            {
                                // RL，证明_ROOT_Temp_test是_ROOT_Temp_testparent_的右子节点，在AVL树抽象图上就是折线型的
                                right_left_revolve(adjust_reference_parent_node);
                            }
                        }
                        else if (adjust_reference_parent_node->balance_factor_ == -2)
                        {
                            if (adjust_reference_node->balance_factor_ == -1)
                            {
                                // R，说明_ROOT_Temp_test是_ROOT_Temp_testparent_的右子节点，线形
                                right_revolve(adjust_reference_parent_node);
                            }
                            else
                            {
        ...
        container_node *find(const avl_tree_type_k &key_data)
        {
            ...
            while (reference_node != nullptr)
            {
                if (reference_node->data_.first == key_data)
                {
                    break;
                }
                else if (function_policy(reference_node->data_.first, key_data))
                {
                    reference_node = reference_node->right_;
                }
                else
        ...
        balance_tree &pop(const avl_tree_type_k &key_data)
        {
            ...
            while (reference_node != nullptr)
            {
                ...
                parent_node = reference_node;
                if (function_policy(reference_node->data_.first, key_data))
                {
                    reference_node = reference_node->right_;
                }
                else
            ...
            bool isleft_child = (parent_node != nullptr) && (parent_node->left_ == reference_node);

            // 三种情况：左空，右空，左右都不空
            if (reference_node->left_ == nullptr)
            {
                container_node *child = reference_node->right_;
                if (child != nullptr)
                {
                    child->parent_ = parent_node;
                }
            ...
            {
                container_node *child = reference_node->left_;
                if (child != nullptr)
                {
                    child->parent_ = parent_node;
                }
            ...
            {
                // 找右子树最左节点
                container_node *right_subtree_smallest_node = reference_node->right_;
                container_node *smallest_parent_node = reference_node;

                while (right_subtree_smallest_node->left_ != nullptr)
                {
                    smallest_parent_node = right_subtree_smallest_node;
                    right_subtree_smallest_node = right_subtree_smallest_node->left_;
                }

                // 交换数据
                standard_con::algorithm::swap(right_subtree_smallest_node->data_, reference_node->data_);

                // 更新要删除的节点信息
                reference_node = right_subtree_smallest_node;
                parent_node = smallest_parent_node;
                isleft_child = (parent_node->left_ == reference_node);
                ...
                if (parent_node->left_ == reference_node)
                {
                    parent_node->left_ = child;
                }
                else
            ...
            while (current != nullptr)
            {
                ...
                // 平衡因子调整逻辑
                if (current->balance_factor_ == 0)
                {
                    // 高度未变，不需要继续调整
                    break;
                }
                else if (current->balance_factor_ == 1 || current->balance_factor_ == -1)
                {
                    // 高度变化，但不需要旋转，继续向上调整
                    isleft_child = (current->parent_ != nullptr) && (current->parent_->left_ == current);
                    current = current->parent_;
                }
                else if (current->balance_factor_ == 2 || current->balance_factor_ == -2)
                {
                    // 需要旋转调整
                    if (current->balance_factor_ == 2)
                    {
                        container_node *child = current->right_;
                        if (child->balance_factor_ == 1)
                        {
                    ...
                    else // current->balance_factor_ == -2
                    {
                        container_node *child = current->left_;
                        if (child->balance_factor_ == -1)
                        {
