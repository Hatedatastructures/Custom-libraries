...
namespace bloom_filter_container
{
    ...
    public:
        ...
        {
            capacity_ = 1000;
            instance_bit_set.resize(capacity_);
        }
        ...
        {
            uint64_t primary_mapping_location = hash_functions_object.hash_sdmmhash(temp_bf_map_value) % capacity_;
            uint64_t secondary_mapping_location = hash_functions_object.hash_djbhash(temp_bf_map_value) % capacity_;
            uint64_t tertiary_mapping_location = hash_functions_object.hash_pjwhash(temp_bf_map_value) % capacity_;
