...
namespace standard_con::pointer
{
    ...
    */
    ...
    public:
        ...
        {
            ptr_ = smart_ptr_data.ptr_;
            smart_ptr_data.ptr_ = nullptr;
        }
    ...
    public:
        ...
        unique_ptr<unique_ptr_type> &operator=(unique_ptr<unique_ptr_type, deleter> &&unique_ptr_data) noexcept
        {
            if (&unique_ptr_data != this)
            {
                ...
                ptr_ = unique_ptr_data.ptr_;
                unique_ptr_data.ptr_ = nullptr;
            }
            return *this;
        }
    ...
    */
    ...
    private:
        ...
        void release() noexcept
        {
            bool flag = false;
            {
                std::lock_guard<std::mutex> guard(*pmutex_);
                if (--(*shared_count_) == 0 && ptr_ != nullptr)
                {
                    deleter_(ptr_);
                    ptr_ = nullptr;
        ...
        {
            ptr_ = deliver_value.ptr_;
            shared_count_ = deliver_value.shared_count_;
            pmutex_ = deliver_value.pmutex_;
            deleter_ = deliver_value.deleter_;
        }
    ...
    public:
        ...
        {
            assignment(shared_ptr_data);
            shared_ptr_data.ptr_ = nullptr;
            shared_ptr_data.shared_count_ = new int(1);
            shared_ptr_data.pmutex_ = new std::mutex;
        }
        ~shared_ptr() noexcept
        {
            release();
        ...
        {
            return shared_count_ ? *shared_count_ : -1;
        }
        Ref operator*() noexcept
        {
            return *(ptr_);
    ...
    */
    ...
    private:
        weak_ptr_type *ptr_;
        pointer::shared_ptr<weak_ptr_type> *shared_ptr_;
        using Ref = weak_ptr_type &;
        using ptr = weak_ptr_type *;

    public:
        weak_ptr() noexcept
                : ptr_(nullptr), shared_ptr_(nullptr) {}
        explicit weak_ptr(shared_ptr<weak_ptr_type> &weak_ptr_data) noexcept
                : ptr_(weak_ptr_data.get_ptr()), shared_ptr_(&weak_ptr_data) {}
        weak_ptr(const weak_ptr &weak_ptr_data) noexcept
                : ptr_(weak_ptr_data.ptr_), shared_ptr_(weak_ptr_data.shared_ptr_) {}
        weak_ptr(weak_ptr &&weak_ptr_data) noexcept
                : ptr_(weak_ptr_data.ptr_), shared_ptr_(weak_ptr_data.shared_ptr_)
        {
            weak_ptr_data.ptr_ = nullptr;
            weak_ptr_data.shared_ptr_ = nullptr;
        }
        ...
        weak_ptr &operator=(const weak_ptr &weak_ptr_data) noexcept
        {
            ...
            {
                ptr_ = weak_ptr_data.ptr_;
                shared_ptr_ = weak_ptr_data.shared_ptr_;
            }
            return *this;
        }
        ...
        {
            ptr_ = shared_ptr_data.get_ptr();
            shared_ptr_ = &shared_ptr_data;
            return *this;
        }
        ~weak_ptr() noexcept
        {
        ...
        {
            return shared_ptr_->get_count() == 0;
        }
        int get_count() const noexcept
        {
            return shared_ptr_ ? shared_ptr_->get_count() : -1;
        }
    };
}