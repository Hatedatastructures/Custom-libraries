...
namespace list_container
{
    /*
            ...
            *   - 包含前驱指针 `prev_`、后继指针 `next_` 和元素数据 `data_`
            *   - 支持拷贝构造和移动构造初始化
            *
            ...
            * * - `head_`: 哨兵节点指针，作为链表的头节点（不存储实际数据）
            *   - 初始化时 `prev_` 和 `next_` 均指向自身，形成循环结构
    ...
    */
    ...
    class list
    {
        ...
        {
            // 节点类
            list_container_node<list_type_function_node> *prev_;
            list_container_node<list_type_function_node> *next_;
            list_type_function_node data_;

            explicit list_container_node(const list_type_function_node &list_type_data = list_type_function_node())
                    : prev_(nullptr), next_(nullptr), data_(list_type_data)
            {
                // 列表初始化
            }
        ...
        public:
            ...
            {
                // 先用在加
                list_iterator return_self(node_);
                node_ = node_->next_;
                // 把本体指向下一个位置
                return return_self;
            }
            ...
            {
                // 比较两个指针及其上一个和下一个指针地址
                return node_ != IteratorTemp.node_;
            }
        ...
        public:
            ...
            {
                reverse_list_iterator _temp(it_);
                --it_;
                return _temp;
            }
            reverse_list_iterator &operator--() noexcept
        ...
        container_node *head_;
        //head_为哨兵位
        void create_head()
        {
            try
            {
                head_ = new container_node;
                head_->prev_ = head_;
                head_->next_ = head_;
            }
    ...
    public:
        ...
        list(iterator first, iterator last)
        {
            try
            {
                if (first.node_ == nullptr || last.node_ == nullptr)
                {
        ...
        {
            create_head(); // 移动构造
            head_ = std::move(list_data.head_);
            list_data.head_ = nullptr;
        }
        void swap(list_container::list<list_type> &swap_target) noexcept
        {
            standard_con::algorithm::swap(head_, swap_target.head_);
        }
        ...
        {
            container_node *current_node = head_->next_;
            uint64_t count = 0;
        ...
        */
        ...
        iterator insert(iterator iterator_position, const list_type &list_type_data)
        {
            try
            {
                if (iterator_position.node_ == nullptr)
                {
                    throw custom_exception::fault("传入迭代器参数为空", "list::insert", __LINE__);
                }
                auto *new_container_node(new container_node(list_type_data));
                // 开辟新节点
                container_node *iterator_current_node = iterator_position.node_;
                // 保存pos位置的值
                new_container_node->prev_ = iterator_current_node->prev_;
                new_container_node->next_ = iterator_current_node;
                new_container_node->prev_->next_ = new_container_node;
                iterator_current_node->prev_ = new_container_node;
        ...
        iterator erase(iterator iterator_position)
        {
            try
            {
                ...
                container_node *iterator_delete_node = iterator_position.node_;  // 找到待删除的节点
                container_node *next_element_node = iterator_delete_node->next_; // 保存下一个节点的位置

                iterator_delete_node->prev_->next_ = iterator_delete_node->next_; // 将该节点从链表中拆下来并删除
                iterator_delete_node->next_->prev_ = iterator_delete_node->prev_;
        ...
        void clear() noexcept
        {
            ...
            while (current_node != head_)
            {
                head_->next_ = current_node->next_;
                delete current_node;
