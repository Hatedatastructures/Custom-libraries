...
namespace base_container
{
    ...
    */
    ...
    private:
        ...
        {
        public:
            rb_tree_type_value data_;
            rb_tree_node *left_;
            rb_tree_node *right_;
            rb_tree_node *parent_;
            rb_tree_color color_;
            explicit rb_tree_node(const rb_tree_type_value &val_data = rb_tree_type_value())
                    : data_(val_data), left_(nullptr), right_(nullptr), parent_(nullptr), color_(rb_tree_color::red)
            {
                ;
            }
            explicit rb_tree_node(rb_tree_type_value &&val_data) noexcept
                    : data_(std::move(val_data)), left_(nullptr), right_(nullptr), parent_(nullptr), color_(rb_tree_color::red)
            {
            }
        };
        ...
        public:
            ...
            rb_tree_iterator()
            {
                node_iterator_ptr_ = nullptr;
            }
            Ref &operator*()
            {
                return node_iterator_ptr_->data_;
            }
            Ptr operator->()
            {
                return &(node_iterator_ptr_->data_);
            ...
            self &operator++()
            {
                if (node_iterator_ptr_ == nullptr)
                {
                    return *this;
                }
                if (node_iterator_ptr_->right_ != nullptr)
                {
                    iterator_node *subtree_node = node_iterator_ptr_->right_;
                    while (subtree_node->left_ != nullptr)
                    {
                        subtree_node = subtree_node->left_;
                    }
                    node_iterator_ptr_ = subtree_node;
                }
                else
                {
                    // 代表右子树已经走完，需要向上遍历，继续向上找右子树，如果停下来，说明走完整棵树或者是走到根节点
                    iterator_node *last_parent_node = node_iterator_ptr_->parent_;
                    iterator_node *temporary_subtree_node = node_iterator_ptr_;
                    while (last_parent_node != nullptr && temporary_subtree_node == last_parent_node->right_)
                    {
                        temporary_subtree_node = last_parent_node;
                        last_parent_node = last_parent_node->parent_;
                    }
                    node_iterator_ptr_ = last_parent_node;
            ...
            self &operator--()
            {
                ...
                {
                    iterator_node *subtree_node = node_iterator_ptr_->left_;
                    while (subtree_node->right_ != nullptr)
                    {
                        subtree_node = subtree_node->right_;
                    }
                    node_iterator_ptr_ = subtree_node;
                }
                else
                {
                    iterator_node *last_parent_node = node_iterator_ptr_->parent_;
                    iterator_node *temporary_subtree_node = node_iterator_ptr_;
                    while (last_parent_node != nullptr && temporary_subtree_node == last_parent_node->left_)
                    {
                        temporary_subtree_node = last_parent_node;
                        last_parent_node = last_parent_node->parent_;
                    }
            ...
            {
                return node_iterator_ptr_ == it_data.node_iterator_ptr_;
            }
            bool operator!=(const self &it_data) const
            {
                return node_iterator_ptr_ != it_data.node_iterator_ptr_;
        ...
        void left_revolve(container_node *subtree_node)
        {
            ...
            // 防止空指针解引用
            subtree_node->right_ = sub_right_left_node;
        ...
        void clear(container_node *clear_node_ptr) noexcept
        {
            ...
            else
            {
                ...
                while (!resource_cleanup_stack.empty())
                {
                    ...
                    if (clear_node_ptr->right_ != nullptr)
                    {
                        resource_cleanup_stack.push(clear_node_ptr->right_);
                    }
                    if (clear_node_ptr->left_ != nullptr)
                    {
                        resource_cleanup_stack.push(clear_node_ptr->left_);
                    }
        ...
        void interior_middle_order_traversal(container_node *intermediate_traversal_node)
        {
            ...
            while (intermediate_traversal_node != nullptr || !interior_stack.empty())
            {
                while (intermediate_traversal_node != nullptr)
                {
                    ...
                    intermediate_traversal_node = intermediate_traversal_node->left_;
                }
                intermediate_traversal_node = interior_stack.top();
                interior_stack.pop();
                std::cout << intermediate_traversal_node->data_ << " ";
                // std::cout << intermediate_traversal_node->color_ <<" ";
                intermediate_traversal_node = intermediate_traversal_node->right_;
            }
        }
        void interior_pre_order_traversal(container_node *preamble_traversal_node)
        {
            ...
            while (!interior_stack.empty())
            {
                ...
                // std::cout << reference_node->color_ <<" ";
        ...
        {
            return current_node == nullptr ? rb_tree_color::black : current_node->color_;
        }
    ...
    public:
        ...
        {
            root_ = new container_node(rb_tree_data);
            root_->color_ = rb_tree_color::black;
        }
        explicit red_black_tree(rb_tree_type_value &&rb_tree_data) noexcept
        {
            root_ = new container_node(std::forward<rb_tree_type_value>(rb_tree_data));
            root_->color_ = rb_tree_color::black;
        }
        ...
        {
            root_ = std::move(rb_tree_data.root_);
            rb_tree_data.root_ = nullptr;
        }
        red_black_tree(const red_black_tree &rb_tree_data)
                : root_(nullptr), element(rb_tree_data.element), function_policy(rb_tree_data.function_policy)
        {
            if (rb_tree_data.root_ == nullptr)
            {
                root_ = nullptr;
            }
            else
            {
                ...
                // 创建根节点
                root_ = new container_node(rb_tree_data.root_->data_);
                root_->color_ = rb_tree_data.root_->color_;
                root_->parent_ = nullptr; // 根节点的父节点为nullptr

                // 初始化栈，将根节点的子节点压入（注意：这里父节点是 _ROOT，一级指针）
                if (rb_tree_data.root_->right_ != nullptr)
                {
                    stack.push(standard_con::pair<container_node *, container_node *>(rb_tree_data.root_->right_, root_));
                }
                if (rb_tree_data.root_->left_ != nullptr)
                {
                    stack.push(standard_con::pair<container_node *, container_node *>(rb_tree_data.root_->left_, root_));
                }
                ...
                while (!stack.empty())
                {
                    ...
                    new_structure_node->color_ = first_node->color_;

                    // 设置父节点关系（注意：parent_node 是一级指针）
                    new_structure_node->parent_ = parent_node;
                    ...
                    if (isleft_child)
                    {
                        parent_node->left_ = new_structure_node;
                    }
                    else
                    {
                        parent_node->right_ = new_structure_node;
                    }
        ...
        red_black_tree &operator=(const red_black_tree rb_tree_data)
        {
            ...
            {
                clear(root_);
                standard_con::algorithm::swap(rb_tree_data.root_, root_);
        ...
        return_pair_value push(const rb_tree_type_value &value_data)
        {
            if (root_ == nullptr)
            {
                root_ = new container_node(value_data);
                root_->color_ = rb_tree_color::black;
                return return_pair_value(iterator(root_), true);
            }
            else
            {
                container_node *reference_node = root_;
                ...
                while (reference_node != nullptr)
                {
                    ...
                    if (!function_policy(element(reference_node->data_), element(value_data)) &&
                            !function_policy(element(value_data), element(reference_node->data_)))
                    {
                        // 插入失败，找到相同的值，开始返回
                        return return_pair_value(iterator(reference_node), false);
                    }
                    else if (function_policy(element(reference_node->data_), element(value_data)))
                    {
                ...
                if (function_policy(element(parent_node->data_), element(reference_node->data_)))
                {
                ...
                reference_node->color_ = rb_tree_color::red;
                reference_node->parent_ = parent_node;
                container_node *return_push_node = reference_node;
                // 保存节点
                // 开始调整，向上调整颜色节点
                while (parent_node != nullptr && parent_node->color_ == rb_tree_color::red)
                {
                    container_node *grandfther_node = parent_node->parent_;
                    if (grandfther_node->left_ == parent_node)
                    {
                        // 叔叔节点
                        //  std::cout << "push" <<" ";
                        container_node *uncle_node = grandfther_node->right_;
                        // 情况1：uncle存在，且为红
                        // 情况2: uncle不存在，那么_ROOT_Temp就是新增节点
                        // 情况3：uncle存在且为黑，说明_ROOT_Temp不是新增节点
                        if (uncle_node && uncle_node->color_ == rb_tree_color::red)
                        {
                            // 情况1：
                            parent_node->color_ = uncle_node->color_ = rb_tree_color::black;
                            grandfther_node->color_ = rb_tree_color::red;
                            // 颜色反转完成
                            reference_node = grandfther_node;
                            parent_node = reference_node->parent_;
                            // 向上调整,继续从红色节点开始
                        }
                        else
                        {
                            // 情况3：该情况双旋转单旋
                            if (reference_node == parent_node->right_)
                            {
                            ...
                            grandfther_node->color_ = rb_tree_color::red;
                            parent_node->color_ = rb_tree_color::black;
                        }
                    }
                    else
                    {
                        container_node *uncle_node = grandfther_node->left_;
                        // 与上面相反
                        if (uncle_node && uncle_node->color_ == rb_tree_color::red)
                        {
                            // 情况1：
                        ...
                        {
                            // 情况3：该情况双旋转单旋
                            if (reference_node == parent_node->left_)
                            {
        ...
        return_pair_value push(rb_tree_type_value &&value_data) noexcept
        {
            if (root_ == nullptr)
            {
                root_ = new container_node(std::forward<rb_tree_type_value>(value_data));
                root_->color_ = rb_tree_color::black;
                return return_pair_value(iterator(root_), true);
            }
            else
        ...
        */
        void delete_adjust(container_node *current_node, container_node *parent)
        {
            ...
            while (current_node != root_ && (current_node == nullptr || black_get(current_node)))
            {
                if (current_node == root_)
                {
                    break;
                }
                if (parent->left_ == current_node)
                {
                    container_node *brother = parent->right_;
                    if (red_get(brother))
                    {
                        // 情况1：兄弟节点为红
                        brother->color_ = rb_tree_color::black;
                        parent->color_ = rb_tree_color::red;
                        left_revolve(parent);
                        // 调整后，兄弟节点为黑
                        // 继续向下调整
                        brother = parent->right_;
                    }
                    if ((brother != nullptr && black_get(brother)) && (brother->left_ == nullptr || (brother->left_)) &&
                            (brother->right_ == nullptr || black_get(brother->right_)))
                    {
                        // 情况2：兄弟节点为黑，且兄弟节点两个子节点都为黑
                        brother->color_ = rb_tree_color::red;
                        current_node = parent;
                        parent = current_node->parent_;
                        if (current_node->color_ == rb_tree_color::red)
                        {
                            current_node->color_ = rb_tree_color::black;
                            break;
                        }
                    }
                    else if ((brother != nullptr && black_get(brother)) && (brother->right_ == nullptr || black_get(brother->right_)) &&
                                      (brother->left_ != nullptr && red_get(brother->left_)))
                    {
                        // 情况3：兄弟节点为黑，兄弟节点左节点为红，右节点为黑
                        brother->left_->color_ = rb_tree_color::black;
                        brother->color_ = rb_tree_color::red;
                    ...
                    else if ((brother != nullptr || black_get(brother)) && (brother->right_ != nullptr && red_get(brother->right_)))
                    {
                        // 情况4：兄弟节点为黑，兄弟节点右节点为红
                        brother->color_ = parent->color_;
                        parent->color_ = rb_tree_color::black;
                        brother->right_->color_ = rb_tree_color::black;
                        left_revolve(parent);
                        current_node = root_;
                        parent = current_node->parent_;
                    }
                ...
                {
                    container_node *brother = parent->left_;
                    if (red_get(brother))
                    {
                        // 情况1：兄弟节点为红
                        brother->color_ = rb_tree_color::black;
                    ...
                    if (brother != nullptr && black_get(brother) && (brother->left_ == nullptr || black_get(brother->left_)) &&
                            (brother->right_ == nullptr || black_get(brother->right_)))
                    {
                        // 情况2：兄弟节点为黑，且兄弟节点两个子节点都为黑
                        brother->color_ = rb_tree_color::red;
                        ...
                        if (current_node->color_ == rb_tree_color::red)
                        {
                            ...
                            break;
                                      ...
                                      (brother->left_ == nullptr || black_get(brother->left_)))
                    {
                        // 情况3：兄弟节点为黑，兄弟节点左节点为红，右节点为黑
                        brother->right_->color_ = rb_tree_color::black;
                        brother->color_ = rb_tree_color::red;
                    ...
                    else if (brother != nullptr && black_get(brother) && brother->left_ != nullptr && red_get(brother->left_))
                    {
                        // 情况4：兄弟节点为黑，兄弟节点右节点为红
                        brother->color_ = parent->color_;
                        parent->color_ = rb_tree_color::black;
        ...
        return_pair_value pop(const rb_tree_type_value &rb_tree_data)
        {
            ...
            else
            {
                ...
                while (reference_node != nullptr)
                {
                    if (!function_policy(element(reference_node->data_), element(rb_tree_data)) && !function_policy(element(rb_tree_data), element(reference_node->data_)))
                    {
                        break;
                    }
                    // 防止父亲自赋值
                    parent_node = reference_node;
                    if (function_policy(element(reference_node->data_), element(rb_tree_data)))
                    {
                        reference_node = reference_node->right_;
                    }
                    else
                ...
                // 找到位置开始删除
                rb_tree_color delete_color = reference_node->color_;
                if (reference_node->left_ == nullptr)
                {
                    if (reference_node->right_ != nullptr)
                    {
                        // 右节点有值
                        reference_node->right_->parent_ = parent_node;
                    }
                    ...
                    else
                    {
                        ...
                        {
                            parent_node->left_ = reference_node->right_;
                        }
                        else
                        {
                            parent_node->right_ = reference_node->right_;
                        }
                    }
                    adjust_node = reference_node->right_;
                ...
                else if (reference_node->right_ == nullptr)
                {
                    if (reference_node->left_ != nullptr)
                    {
                        reference_node->left_->parent_ = parent_node;
                        // 链接父节点
                    }
                    if (parent_node == nullptr)
                    {
                    ...
                    else
                    {
                        if (parent_node->left_ == reference_node)
                        {
                            parent_node->left_ = reference_node->left_;
                        }
                        else
                        {
                            parent_node->right_ = reference_node->left_;
                        }
                    }
                    adjust_node = reference_node->left_;
                    adjust_parent_node = parent_node;
                    delete reference_node;
                    reference_node = nullptr;
                }
                else if (reference_node->right_ != nullptr && reference_node->left_ != nullptr)
                {
                    ...
                    delete_color = right_subtree_smallest_node->color_;

                    // 交换数据 AND 交换颜色
                    standard_con::algorithm::swap(right_subtree_smallest_node->data_, reference_node->data_);
                    standard_con::algorithm::swap(right_subtree_smallest_node->color_, reference_node->color_);

                    // 然后正确地把后继节点的位置接到它父节点上：
                    if (smallest_parent_node->left_ == right_subtree_smallest_node)
                    {
                        smallest_parent_node->left_ = right_subtree_smallest_node->right_;
                    }
                    else
                    {
                        smallest_parent_node->right_ = right_subtree_smallest_node->right_;
                    }
                    if (right_subtree_smallest_node->right_)
                    {
                        right_subtree_smallest_node->right_->parent_ = smallest_parent_node;
                    }
                    adjust_node = right_subtree_smallest_node->right_;
        ...
        iterator find(const rb_tree_type_value &val_data)
        {
            ...
            {
                container_node *root_find_node = root_;
                while (root_find_node != nullptr)
                {
                    if (!function_policy(element(root_find_node->data_), element(val_data)))
                    {
                        return iterator(root_find_node);
                    }
                    else if (function_policy(element(root_find_node->data_), element(val_data)))
                    {
                        root_find_node = root_find_node->right_;
                    }
                    else
                    {
                        root_find_node = root_find_node->left_;
                    }
                }
                return iterator(nullptr);
            }
        ...
        iterator begin()
        {
            container_node *root_node = root_;
            while (root_node != nullptr && root_node->left_ != nullptr)
            {
        ...
        {
            container_node *croot_node = root_;
            while (croot_node != nullptr && croot_node->left_ != nullptr)
            {
                croot_node = croot_node->left_;
            }
        ...
        {
            container_node *iterator_node = root_;
            while (iterator_node != nullptr && iterator_node->right_ != nullptr)
            {
                iterator_node = iterator_node->right_;
            }
    ...
    */
    ...
    class hash_table
    {
        ...
        public:
            ...
            {
                data_ = std::move(hash_table_value_data);
                next_ = nullptr;
                overall_list_prev = nullptr;
                overall_list_next = nullptr;
            }
        ...
        public:
            ...
            Ref operator*() { return hash_table_iterator_node->data_; }

            Ptr operator->() { return &hash_table_iterator_node->data_; }
        ...
        void hash_chain_adjustment(container_node *&provisional_parent_node, container_node *&provisional_node, uint64_t &hash_map_location)
        {
            ...
            {
                // 父亲节点不为空，防止空指针错误
                provisional_parent_node->next_ = provisional_node->next_;
            }
            else
            {
                // 父亲节点为空，直接将映射位置置空
    ...
    public:
        ...
        hash_table(const hash_table &hash_table_data)
                : value_imitation_functions(hash_table_data.value_imitation_functions), size_(hash_table_data.size_), load_factor(hash_table_data.load_factor),
                    ...
                    overall_list_head_node(nullptr)
        {
            ...
            for (uint64_t new_hash_container_capacity = 0; new_hash_container_capacity < hash_capacity; ++new_hash_container_capacity)
            {
                ...
                {
                    // 2.1 创建新节点并拷贝数据
                    auto *new_structure_node = new container_node(src_bucket_node->data_);
                    // 2.2 插入到“桶内部”链表
                    if (last_in_bucket != nullptr)
                    {
                        last_in_bucket->next_ = new_structure_node;
                    }
                    else
                    {
                    ...
                    // 继续下一个源节点
                    src_bucket_node = src_bucket_node->next_;
                }
            }
            if (overall_list_before_node != nullptr)
            {
        ...
        ~hash_table() noexcept
        {
            for (uint64_t i = 0; i < vector_hash_table.size(); ++i)
            {
                ...
                while (hash_bucket_delete != nullptr)
                {
                    ...
                    hash_bucket_delete = hash_bucket_delete->next_;
        ...
        iterator operator[](const hash_table_type_key &key_value)
        {
            ...
            else
            {
                ...
                {
                    if (value_imitation_functions(bucket_node->data_) == value_imitation_functions(key_value))
                    {
        ...
        bool push(const hash_table_type_value &hash_table_value_data)
        {
            ...
            // 判断扩容
            if (size_ * 10 >= hash_capacity * load_factor)
            {
                // 扩容
                ...
                {
                    uint64_t new_mapping_value = hash_function_object(start_position_node->data_) % new_container_capacity;
                    // 重新计算映射值
                    container_node *hash_bucket_node = new_vector_hash_table[new_mapping_value];
                    if (hash_bucket_node == nullptr)
                    {
                        auto *new_mapping_data = new container_node(start_position_node->data_);
                    ...
                    else
                    {
                        ...
                        new_mapping_data->next_ = hash_bucket_node;
                        new_vector_hash_table[new_mapping_value] = new_mapping_data;
                        // 头插节点
                    }
                    ++new_size;
            ...
            if (size_ == 0 && overall_list_head_node == nullptr)
            {
                new_mapping_data->overall_list_prev = nullptr;
        ...
        bool push(hash_table_type_value &&hash_table_value_data) noexcept
        {
            ...
            if (size_ * 10 >= hash_capacity * load_factor)
            {
                ...
                while (start_position_node != nullptr)
                {
                    ...
                    if (hash_bucket_node == nullptr)
                    {
                        auto *new_mapping_data = new container_node(std::forward<hash_table_type_value>(start_position_node->data_));
                        if (regional_list_head_node == nullptr)
                        {
                            new_mapping_data->overall_list_prev = nullptr;
                            new_mapping_data->overall_list_next = nullptr;
        ...
        bool pop(const hash_table_type_value &hash_table_value_data)
        {
            ...
            {
                // 找到位置
                if (value_imitation_functions(hash_bucket_node->data_) == value_imitation_functions(hash_table_value_data))
                {
                ...
                hash_bucket_node = hash_bucket_node->next_;
                // 向下遍历
            }
            return false;
        }
