...
namespace vector_container
{
    /*
      * @brief  #### `vector` 类模板
            ...
            * * - `data_pointer_`: 指向动态数组的起始地址
            *
            * * - `size_pointer_`: 指向数组中最后一个元素的下一个位置（表示当前元素数量）
            *
            * * - `capacity_pointer_`: 指向数组容量的末尾位置（表示可容纳的最大元素数量）
    ...
    */
    ...
    private:
        iterator data_pointer_;     // 指向数据的头
        iterator size_pointer_;     // 指向数据的尾
        iterator capacity_pointer_; // 指向容量的尾
    public:
        ...
        {
            return size_pointer_;
        }
        [[nodiscard]] uint64_t size() const noexcept
        {
            return data_pointer_ ? (size_pointer_ - data_pointer_) : 0;
        }

        [[nodiscard]] uint64_t capacity() const noexcept
        {
            return data_pointer_ ? (capacity_pointer_ - data_pointer_) : 0;
        }
        ...
        {
            return *(size_pointer_ - 1);
        }

        vector() noexcept
        {
            data_pointer_ = nullptr;
            size_pointer_ = nullptr;
            capacity_pointer_ = nullptr;
        }
        explicit vector(const uint64_t &container_capacity, const vector_type &vector_data = vector_type())
                : data_pointer_(new vector_type[container_capacity]), size_pointer_(data_pointer_ + container_capacity), capacity_pointer_(data_pointer_ + container_capacity)
        {
            for (uint64_t corresponding_location = 0; corresponding_location < container_capacity; corresponding_location++)
            {
                data_pointer_[corresponding_location] = vector_data;
            }
        }
        vector(std::initializer_list<vector_type> lightweight_container)
                : data_pointer_(new vector_type[lightweight_container.size()]), size_pointer_(data_pointer_ + lightweight_container.size()), capacity_pointer_(data_pointer_ + lightweight_container.size())
        {
            // 链式拷贝
            uint64_t corresponding_location = 0;
            for (auto &chained_values : lightweight_container)
            {
                data_pointer_[corresponding_location] = std::move(chained_values);
        ...
        vector_type &find(const uint64_t &find_size)
        {
            try
            {
                ...
                {
                    return data_pointer_[find_size];
                }
            }
        ...
        vector<vector_type> &size_adjust(const uint64_t &data_size, const vector_type &padding_temp_data = vector_type())
        {
            ...
            if (data_size > container_capacity)
            {
                ...
                {
                    data_pointer_[assignment_traversal] = padding_temp_data;
                }
            }
            else
            {
                ...
                {
                    size_pointer_ = data_pointer_ + data_size;
                }
            }
            return *this;
        }
        vector(const vector<vector_type> &vector_data)
                : data_pointer_(vector_data.capacity() ? new vector_type[vector_data.capacity()] : nullptr),
                    size_pointer_(data_pointer_ + vector_data.size()), capacity_pointer_(data_pointer_ + vector_data.capacity())
        {
            for (uint64_t copy_assignment_traversal = 0; copy_assignment_traversal < vector_data.size(); copy_assignment_traversal++)
            {
                data_pointer_[copy_assignment_traversal] = vector_data.data_pointer_[copy_assignment_traversal];
            }
        }
        vector(vector<vector_type> &&vector_data) noexcept
        {
            data_pointer_ = std::move(vector_data.data_pointer_);
            size_pointer_ = std::move(vector_data.size_pointer_);
            capacity_pointer_ = std::move(vector_data.capacity_pointer_);
            vector_data.data_pointer_ = vector_data.size_pointer_ = vector_data.capacity_pointer_ = nullptr;
        }
        ~vector() noexcept
        {
            delete[] data_pointer_;
            data_pointer_ = size_pointer_ = capacity_pointer_ = nullptr;
        }
        void swap(vector<vector_type> &vector_data) noexcept
        {
            standard_con::algorithm::swap(data_pointer_, vector_data.data_pointer_);
            standard_con::algorithm::swap(size_pointer_, vector_data.size_pointer_);
            standard_con::algorithm::swap(capacity_pointer_, vector_data.capacity_pointer_);
        }
        iterator erase(iterator delete_position) noexcept
        {
            // 删除元素
            iterator next_position = delete_position + 1;
            while (next_position != size_pointer_)
            {
        ...
        vector<vector_type> &resize(const uint64_t &new_container_capacity, const vector_type &vector_data = vector_type())
        {
            try
            {
                ...
                if (static_cast<uint64_t>(capacity_pointer_ - data_pointer_) < new_container_capacity)
                {
                    ...
                    {
                        new_vector_type_array[original_data_traversal] = std::move(data_pointer_[original_data_traversal]);
                    }
                    ...
                    delete[] data_pointer_;
                    data_pointer_ = new_vector_type_array;
                    size_pointer_ = data_pointer_ + original_size; // 使用 original_size 来重建 size_pointer_
                    capacity_pointer_ = data_pointer_ + new_container_capacity;
                }
            }
        ...
        {
            if (size_pointer_ == capacity_pointer_)
            {
                const uint64_t new_container_capacity = data_pointer_ == nullptr ? 10 : static_cast<uint64_t>((capacity_pointer_ - data_pointer_) * 2);
                resize(new_container_capacity);
            }
            // 注意—size_pointer_是原生迭代器指针，需要解引用才能赋值
            *size_pointer_ = vector_type_data;
            ++size_pointer_;
            return *this;
        }
        vector<vector_type> &push_back(vector_type &&vector_type_data)
        {
            ...
            // 注意size_pointer_是原生迭代器指针，需要解引用才能赋值
            *size_pointer_ = std::move(vector_type_data);
            // new (data_pointer_) vector_type(std::forward<vector_type>(vector_type_data));
            ++size_pointer_;
            return *this;
        }
        vector<vector_type> &pop_back()
        {
            if (size_pointer_ > data_pointer_)
            {                  // 至少有一个元素
                --size_pointer_; // 尾指针前移
            }
            return *this;
        ...
        vector<vector_type> &push_front(const vector_type &vector_type_data)
        {
            ...
            if (size_pointer_ == capacity_pointer_)
            {
                const uint64_t new_container_size = data_pointer_ == nullptr ? 10 : static_cast<uint64_t>((capacity_pointer_ - data_pointer_) * 2);
                resize(new_container_size);
            }
            for (uint64_t container_size = size(); container_size > 0; --container_size)
            {
                data_pointer_[container_size] = data_pointer_[container_size - 1];
            }
            *data_pointer_ = vector_type_data;
            ++size_pointer_;
            return *this;
        }
        vector<vector_type> &pop_front()
        {
            if (size() > 0)
            {
                ...
                {
                    data_pointer_[assignment_traversal - 1] = data_pointer_[assignment_traversal];
                }
                --size_pointer_;
            }
            return *this;
        ...
        vector_type &operator[](const uint64_t &access_location)
        {
            try
            {
                ...
                {
                    return data_pointer_[access_location];
                }
            }
            catch (const custom_exception::fault &process)
            {
        ...
        vector<vector_type> &operator=(vector<vector_type> &&vector_mobile_data) noexcept
        {
            ...
            {
                data_pointer_ = std::move(vector_mobile_data.data_pointer_);
                size_pointer_ = std::move(vector_mobile_data.size_pointer_);
                capacity_pointer_ = std::move(vector_mobile_data.capacity_pointer_);
                vector_mobile_data.data_pointer_ = vector_mobile_data.size_pointer_ = vector_mobile_data.capacity_pointer_ = nullptr;
            }
            return *this;
        }
        vector<vector_type> &operator+=(const vector<vector_type> &vector_data)
        {
            if (vector_data.size() == 0 || vector_data.data_pointer_ == nullptr)
            {
                return *this;
            }
            ...
            {
                data_pointer_[slicing_traversal] = vector_data.data_pointer_[array_counter++];
            }
            size_pointer_ = data_pointer_ + (vector_data_size + container_size);
            return *this;
        }
