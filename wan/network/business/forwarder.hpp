...
namespace represents
{
    ...
    private:
        ...
        std::unordered_multimap<std::string, upstream> upstreams_; // 上游代理名单，多IP：key=域名，value=上游配置
        conversation::connection_pool<request, response> http_pool_; // http连接池
        transponder_config config_{}; // 统一配置（可覆盖）

        // 任务管理与执行器
        std::atomic<bool> stopping_{false}; // 停止标志
        std::atomic<std::size_t> active_tasks_{0}; // 当前活跃异步任务数
        std::size_t max_async_tasks_{1024}; // 最大并发异步任务数，防止任务爆炸
        std::mutex tasks_mutex_; // 任务计数互斥
        std::condition_variable tasks_cv_; // 任务计数条件变量
        std::shared_ptr<wan::pool::thread_pool> async_pool_; // 线程池

    public:
        explicit transponder(boost::asio::io_context& io_context, const transponder_config& config = transponder_config())
            : io_context_(io_context), http_pool_(io_context), config_(config) { http_pool_.start(); }
        ...
        { 
            config_.ssl_insecure_skip_verify = v; 
        }
        ...
        void add_upstream(const std::string &domain, const std::string &ip_or_host, std::uint16_t port, bool use_https)
        {
            ...
            upstreams_.emplace(up.domain, up);

            conversation::endpoint_config cfg;
            cfg.host = up.host;
            cfg.port = up.port;
            cfg.session_cfg.enable_ssl_ = up.use_https;
            cfg.session_cfg.tls_server_name_ = up.domain.empty() ? up.host : up.domain;
            cfg.session_cfg.ssl_ca_file_ = config_.ssl_ca_file;
            cfg.session_cfg.ssl_cert_file_ = config_.ssl_cert_file;
            cfg.session_cfg.ssl_key_file_ = config_.ssl_key_file;
            cfg.session_cfg.ssl_insecure_skip_verify_ = config_.ssl_insecure_skip_verify;
            // 应用命名空间级连接池默认配置
        ...
        {
            auto range = upstreams_.equal_range(domain);
            for (auto it = range.first; it != range.second; ++it)
            {
                http_pool_.remove_endpoint(it->second.host, it->second.port);
            }
            upstreams_.erase(domain);
        }
        ...
        bool shutdown(std::chrono::milliseconds timeout = std::chrono::milliseconds{5000})
        {
            ...
            while (active_tasks_.load(std::memory_order_relaxed) > 0)
            {
                if (std::chrono::steady_clock::now() >= deadline)
                    break;
                tasks_cv_.wait_until(lk, deadline, [this]{ return active_tasks_.load(std::memory_order_relaxed) == 0; });
            }
            lk.unlock();

            if (async_pool_ && async_pool_->is_running())
            {
                // 尝试关停外部线程池
                async_pool_->shutdown(timeout);
            }
        ...
        std::future<response> forward_async(request req,request_func request_filter = {},response_func response_filter = {})
        {
            ...
            // 简单并发门限，防止任务爆炸
            while (active_tasks_.load(std::memory_order_relaxed) >= max_async_tasks_)
            {
                std::unique_lock<std::mutex> lk(tasks_mutex_);
                tasks_cv_.wait_for(lk, std::chrono::milliseconds(5), [this]{
                    return active_tasks_.load(std::memory_order_relaxed) < max_async_tasks_ || stopping_.load(std::memory_order_acquire);
                });
                if (stopping_.load(std::memory_order_acquire))
                {
                    std::promise<response> p;
            ...
            auto forward_task = [this, req = std::move(req), request_filter, response_filter]() mutable -> response
            {
                ...
                { 
                    transponder* self; 
                    ~Guard(){ self->active_tasks_.fetch_sub(1, std::memory_order_relaxed); 
                    std::lock_guard<std::mutex> lk(self->tasks_mutex_); 
                    self->tasks_cv_.notify_all(); } 
                };
                Guard g{this};
            ...
            if (async_pool_ && async_pool_->is_running())
            {
                return async_pool_->submit(std::move(forward_task));
            }
    ...
    private:
        ...
        {
            auto range = upstreams_.equal_range(std::string(host_name));
        ...
        response perform_upstream_plain(const request &req, const upstream &up)
        {
            namespace http = boost::beast::http;
            response parsed;
            auto borrowed = http_pool_.borrow(up.host, up.port);
            if (!borrowed)
