...
namespace conversation
{
    ...
    private:
        std::atomic<bool> running_{false}; // 会话管理是否正在运行
        mutable std::shared_mutex sessions_mutex_; // 会话映射的互斥锁

        boost::asio::io_context& io_context_; // io上下文
        boost::asio::steady_timer cleanup_timer_; // 会话清理定时器
        std::chrono::seconds cleanup_interval_{60}; // 会话清理时间间隔
        std::unordered_map<std::string, session_ptr> sessions_map_; // 会话映射

        session_management_config config_; // 默认配置

        std::unique_ptr<thread_pool> thread_pool_; // 线程池
        std::atomic<bool> thread_pool_running_{false}; // 线程池是否正在运行
    private:
        /**
          * @brief 初始化线程池
        ...
        bool _initialize_thread_pool()
        {
            if(!thread_pool_)
            {
                ...
                thread_pool_config.pool_name_ = "session_management";
                thread_pool_config.initial_threads_ = config_.thread_size * 2;
                thread_pool_config.min_threads_ = 2;
                thread_pool_config.max_threads_ = config_.thread_max_size * 4;
                thread_pool_config.core_threads_ = config_.thread_size * 2;
                thread_pool_config.queue_policy_ = rank_strategy::priority;
                thread_pool_config.scheduling_tactics_ = scheduling_tactics::adaptive;
                thread_pool_config.enable_monitoring_ = true;
                thread_pool_config.enable_performance_profiling_ = false;
                thread_pool_ = wan::pool::make_thread_pool(thread_pool_config);
                if (thread_pool_ && thread_pool_->start())
                {
                    thread_pool_running_.store(true);
                    return true;
                }
            }
            return false;
        ...
        {
            if(thread_pool_ && thread_pool_running_.load())
            {
                if(thread_pool_->shutdown(std::chrono::seconds(5)))
                {
                    thread_pool_running_.store(false);
                    return true;
                }
            }
            return false;
        ...
        void _start_cleanup_timer()
        {
            ...
            auto cleanup_function = [this](const boost::system::error_code& ec)
            {
                if(!ec && running_.load())
                {
                    ...
                    if(thread_pool_running_.load() && thread_pool_)
                        thread_pool_->submit_priority(weight::low, _cleanup_task);
                    else
                        _cleanup_task();
                    _start_cleanup_timer();
                }
            };
            cleanup_timer_.async_wait(cleanup_function);
        }
        /**
          * @brief 清理过期会话
          */
        void _cleanup_inactive_sessions()
        {
            ...
            std::vector<std::string> inactive_sessions;
            {
                ...
                for (const auto& pair : sessions_map_)
                {
    ...
    private:
        ...
        std::vector<session_ptr> _all_session(bool only_connected) const
        {
            ...
            {
                std::shared_lock<std::shared_mutex> lock(sessions_mutex_);
                snapshot.reserve(sessions_map_.size());
    ...
    public:
        session_management(boost::asio::io_context& io_context,
            ...
            : io_context_(io_context),cleanup_timer_(io_context_),config_(config)
        {
        ...
        -> session_ptr
        {
            if(socket.is_open())
            {
                session_ptr sess = std::make_shared<fundamental::session<request_t,response_t>>(std::move(socket));
                {
                    ...
                    sessions_map_[session_string_id] = sess;
                }
                return sess;
            }
            return nullptr;
        ...
        {
            std::shared_lock<std::shared_mutex> lock(sessions_mutex_);
            auto it = sessions_map_.find(session_string_id);
            if(it != sessions_map_.end())
                return it->second;
            return nullptr;
        }
        ...
        std::vector<std::string> get_session_ids() const
        {
            ...
            session_ids.reserve(sessions_map_.size());
            for(const auto& pair : sessions_map_)
        ...
        bool add_session(session_ptr session)
        {
            ...
            sessions_map_[session_id] = session;
            return true;
        }
        ...
        {
            std::shared_lock<std::shared_mutex> lock(sessions_mutex_);
            return sessions_map_.find(session_id) != sessions_map_.end();
        }
        ...
        bool with_session(session_ptr sess, operation&& linkage_operation, weight priority = weight::normal)
        {
            ...
            auto shell = [this, sp = std::move(sess), execute_function = std::forward<operation>(linkage_operation)]() mutable
            {
                ...
                boost::asio::dispatch(io_context_, linkage_function);
            };

            if(thread_pool_running_.load() && thread_pool_)
                thread_pool_->submit_priority(priority, std::move(shell));
            else
        ...
        void for_each_session(operation&& linkage_operation, weight priority = weight::normal, bool only_connected = true)
        {
            ...
            if(thread_pool_running_.load() && thread_pool_)
                thread_pool_->submit_priority(priority, std::move(dispatch_function));
            else
        ...
        bool broadcast_bytes(std::string_view data, weight priority = weight::normal, bool only_connected = true)
        {
            ...
            auto dispatch_function = [this, vec = std::move(snapshot), payload]() mutable
            {
                for(auto& sp : vec)
                {
                    ...
                    boost::asio::dispatch(io_context_, std::move(send_fn));
                }
            };
            if(thread_pool_running_.load() && thread_pool_)
                thread_pool_->submit_priority(priority, std::move(dispatch_function));
        ...
        {
            if(thread_pool_running_.load() && thread_pool_)
                return std::optional<pool_statistics>(thread_pool_->getstatistics_());
            return std::nullopt;
        }
    }; // end class 
    ...
    private:
        ...
        std::atomic<bool> running_{false}; // 会话管理器运行状态

        boost::asio::io_context& io_context_;  // IO上下文
        boost::asio::steady_timer check_timer_; // 健康检查定时器

        mutable std::shared_mutex map_mutex_; // 会话连接池互斥锁

        std::unordered_map<endpoint_key, std::shared_ptr<endpoint_pool>, endpoint_hash> endpoint_pools_; // 端点连接池
    private:
        /**
          * @brief 获取指定端点连接池
          * @param host 主机名
        ...
        std::shared_ptr<endpoint_pool> _get_pool(const std::string& host, std::uint16_t port) const
        {
            ...
            std::shared_lock<std::shared_mutex> rlock(map_mutex_);
            auto it = endpoint_pools_.find(key);
            if (it == endpoint_pools_.end())
                return nullptr;
            return it->second;
        }
        /**
        ...
        void _schedule_next_check()
        {
            ...
            check_timer_.expires_after(interval);
        ...
        {
            auto proto = cfg.session_cfg.enable_ssl_ ? 
                fundamental::session_type::SSL_CLIENT : fundamental::session_type::TCP_CLIENT;
            auto sp = std::make_shared<session_internal>(io_context_, proto, cfg.session_cfg);
    ...
    public:
        connection_pool(boost::asio::io_context& io_context)
            :io_context_(io_context), check_timer_(io_context) {}
        ...
        bool stop()
        {
            if(!running_.exchange(false))
                return true;
            check_timer_.cancel();
            std::unique_lock<std::shared_mutex> wlock(map_mutex_);
            for (auto &kv : endpoint_pools_)
            {
                auto pool = kv.second;
                std::unique_lock<std::mutex> lock(pool->mtx);
        ...
        bool add_endpoint(const endpoint_config &cfg)
        {
            ...
            std::unique_lock<std::shared_mutex> wlock(map_mutex_);
            if(endpoint_pools_.find(key) != endpoint_pools_.end())
                return true; // 存在为成功
            auto pool = std::make_shared<endpoint_pool>();
            pool->cfg = cfg;
            endpoint_pools_.emplace(key, pool);
            wlock.unlock();
            _preheat_pool(pool);
            return true;
        }
