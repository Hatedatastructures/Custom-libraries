...
namespace protocol
{
    ...
    private:
        ...
        void _serialize_headers_to_string(std::string &out) const
        {
            ...
            {
                const std::string &value = headers_.at(key);
                out.append(key);
                out.append(": ");
                out.append(value);
                out.append("\r\n");
    ...
    public:
        request_header()
        {
            ...
            protocol_type_ = assist::protocol_type::CUSTOM_TCP;
        }

        const std::string &get_method() const noexcept { return method_; }
        void set_method(const std::string &method) { method_ = method; }

        const std::string &get_target() const noexcept { return target_; }
        void set_target(const std::string &target) { target_ = target; }

        const std::string &get_user_agent() const noexcept { return user_agent_; }
        ...
        void set_timestamp(const std::chrono::system_clock::time_point &timestamp) { timestamp_ = timestamp; }
        ...
        {
            std::size_t estimated_size = method_.size() + target_.size() + 64;
            for (const auto &[key, value] : headers_)
            ...
            result += std::format("{} {} {} {} {} {}\r\n",method_, target_, version_,
            static_cast<std::uint8_t>(checksum_type_),checksum_value_, content_length_);

            if (!user_agent_.empty())
                result += std::format("User-Agent: {}\r\n", user_agent_);
            auto timestamp_ms = std::chrono::duration_cast<std::chrono::milliseconds>(timestamp_.time_since_epoch()).count();
        ...
        protocol::json to_json() const override
        {
            ...
            json_object.set("method", method_);
            json_object.set("target", target_);
            json_object.set("user_agent", user_agent_);
        ...
        bool from_json(const protocol::json &json_object) override
        {
            ...
            try
            {
                method_ = json_object.get<std::string>("method", "");
                target_ = json_object.get<std::string>("target", "");
                user_agent_ = json_object.get<std::string>("user_agent", "");
                auto timestamp_ms = json_object.get<std::int64_t>("timestamp", 0);
                timestamp_ = std::chrono::system_clock::time_point(std::chrono::milliseconds(timestamp_ms));
                return true;
            }
            catch (...)
            {
    ...
    private:
        ...
        std::uint16_t status_code_ = 200;                 // 状态码
        std::string status_message_ = "OK";               // 状态消息
        std::chrono::system_clock::time_point timestamp_; // 时间戳
    private:
        /**
          * @brief 序列化头部字段为字符串
    ...
    public:
        ...
        std::uint16_t get_status_code() const noexcept { return status_code_; }
        void set_status_code(std::uint16_t code) { status_code_ = code; }

        const std::string &getstatus_message_() const noexcept { return status_message_; }
        void setstatus_message_(const std::string &message) { status_message_ = message; }

        const std::string &get_server() const noexcept { return server_; }
        ...
        std::string to_string() const override
        {
            std::size_t estimated_size = status_message_.size() + 64;
            for (const auto &[key, value] : headers_)
                estimated_size += key.size() + value.size() + 4;
            std::string result;
            result.reserve(estimated_size);
            result += std::format("{} {} {} {} {} {}\r\n",version_, status_code_, status_message_,
            static_cast<std::uint8_t>(checksum_type_),checksum_value_, content_length_);
            if (!server_.empty())
        ...
        {
            protocol::json json_object = protocol_header::to_json();
            json_object.set("status_code", status_code_);
            json_object.set("status_message", status_message_);
            json_object.set("server", server_);
            auto timestamp_ms = std::chrono::duration_cast<std::chrono::milliseconds>(timestamp_.time_since_epoch()).count();
            json_object.set("timestamp", timestamp_ms);
            return json_object;
        }
        bool from_json(const protocol::json &json_object) override
        {
            ...
            try
            {
                status_code_ = json_object.get<std::uint16_t>("status_code", 200);
                status_message_ = json_object.get<std::string>("status_message", "OK");
                server_ = json_object.get<std::string>("server", "");
                auto timestamp_ms = json_object.get<std::int64_t>("timestamp", 0);
                timestamp_ = std::chrono::system_clock::time_point(std::chrono::milliseconds(timestamp_ms));
                return true;
            }
    ...
    {
    private:
        mutable header_t header_;               // 请求头（mutable 用于缓存校验和）
        std::string message_;                   // 请求体
        mutable std::string cached_full_;       // 缓存的完整请求字符串
        mutable bool full_cache_valid_ = false; // 完整缓存是否有效
    private:
        /**
          * @brief 清除缓存
          */
        ...
        {
            full_cache_valid_ = false;
        } 
    public:
        request() = default;

        explicit request(const header_t &header) : header_(header) {}

        request(const header_t &header, const std::string &body)
        : header_(header), message_(body) {}

        request(const header_t &header, std::string &&body)
        : header_(header), message_(std::move(body)) {}

        // 拷贝和移动语义
        request(const request &other)
        : header_(other.header_), message_(other.message_) {}

        request(request &&other) noexcept
        : header_(std::move(other.header_)), message_(std::move(other.message_)), 
        cached_full_(std::move(other.cached_full_)), full_cache_valid_(other.full_cache_valid_)
        {
            other.full_cache_valid_ = false;
        }
        ...
        request &operator=(request &&other) noexcept
        {
            if (this != &other)
            {
                ...
                cached_full_ = std::move(other.cached_full_);
                full_cache_valid_ = other.full_cache_valid_;
                other.full_cache_valid_ = false;
            }
            return *this;
        }
        const header_t &header() const noexcept { return header_; }
        ...
        const std::string &body() const noexcept { return message_; }
        ...
        {
            if (!full_cache_valid_)
            { // `calculate_and_set_checksum` 作用： 计算消息体校验码并设置校验码
                header_.calculate_and_set_checksum(message_);

                // 优化字符串拼接性能，预分配内存避免多次重新分配
                const auto& header_str = header_.to_string();
                cached_full_.clear();
                cached_full_.reserve(header_str.size() + message_.size());
                cached_full_.append(header_str);
                cached_full_.append(message_);
                full_cache_valid_ = true;
            }
            return cached_full_;
        }
        /**
        ...
        bool from_string(std::string_view data)
        {
            ...
            if (!header_.from_string(header_data))
                return false;
            // 提取body
            std::size_t body_start = header_end + 4; // 跳过 "\r\n\r\n"
            if (body_start < data.size())
                message_ = std::string(data.substr(body_start));
            else
                message_.clear();
            _invalidate_cache();
            return header_.verify_integrity(message_);
        }
        /**
          * @brief 转换为JSON
          * @return JSON对象
        ...
        protocol::json to_json() const
        {
            ...
            header_.calculate_and_set_checksum(message_);
            json_object.set("header", header_.to_json().value());
            json_object.set("body", message_);
            return json_object;
        }
        /**
          * @brief 从`JSON`反序列化
        ...
        bool from_json(const protocol::json &json_object)
        {
            try
            {
                ...
                if (!header_json_str.empty())
                {
                    ...
                    if (!header_.from_json(header_json))
                        return false;
                }
                message_ = json_object.get<std::string>("body", "");
                _invalidate_cache();
                return true;
            }
            catch (...)
        ...
        {
            return header_.to_string() == other.header_.to_string() && message_ == other.message_;
        }
    ...
    {
    private:
        mutable header_t header_;               // 响应头（mutable 用于缓存校验和）
        std::string message_;                   // 响应体
        mutable std::string cached_full_;       // 缓存的完整响应字符串
        mutable bool full_cache_valid_ = false; // 完整缓存是否有效
    ...
    public:
        ...
        explicit response(const header_t &header) : header_(header) {}
        ...
        // 便利构造函数（用于快速创建响应）
        response(std::uint16_t status_code, const std::string &statusmessage_, const std::string &body = "")
        : message_(body)
        {
            if constexpr (std::is_same_v<header_t, response_header>)
            {
                header_.set_status_code(status_code);
                header_.setstatus_message_(statusmessage_);
            }
        }
        ...
        : header_(std::move(other.header_)), message_(std::move(other.message_)), cached_full_(std::move(other.cached_full_)), full_cache_valid_(other.full_cache_valid_)
        {
            other.full_cache_valid_ = false;
        }
        ...
        {
            return header_.get_status_code();
        }
        ...
        {
            header_.set_status_code(code);
            _invalidate_cache();
        }

        template <typename current_header_t = header_t>
        std::enable_if_t<std::is_same_v<current_header_t, response_header>, const std::string &>
        getstatus_message_() const noexcept
        {
            return header_.getstatus_message_();
        }

        template <typename current_header_t = header_t>
        std::enable_if_t<std::is_same_v<current_header_t, response_header>, void>
        setstatus_message_(const std::string &message)
        {
            header_.setstatus_message_(message);
            _invalidate_cache();
        }
        ...
        static response create_success(const std::string &body = "", const std::string &server = "")
        {
            ...
            {
                if (!server.empty())
                    resp.header_.set_server(server);
            }
            return resp;
        }
...
bool protocol::request_header::from_string(std::string_view data)
{
    ...
    std::uint32_t temp_version = 0, tempchecksum_value_ = 0, tempcontent_length_ = 0;
    assist::checksum_type tempchecksum_type_ = assist::checksum_type::CRC32;
    ...
    if (!safe_parse(parts[2], temp_version) || !safe_parse(parts[3], ctype_val) ||
            !safe_parse(parts[4], tempchecksum_value_) || !safe_parse(parts[5], tempcontent_length_))
        return false;
    ...
    tempchecksum_type_ = static_cast<assist::checksum_type>(ctype_val);
    pos = le + 2;
    ...
    // 所有解析成功，提交更改（异常安全）
    headers_ = std::move(temp_headers);
    method_ = std::move(temp_method);
    target_ = std::move(temp_target);
    user_agent_ = std::move(temp_user_agent);
    version_ = temp_version;
    checksum_value_ = tempchecksum_value_;
    content_length_ = tempcontent_length_;
    checksum_type_ = tempchecksum_type_;
    timestamp_ = temp_timestamp;
...
bool protocol::response_header::from_string(std::string_view data)
{
    ...
    std::uint8_t ctype_val;
    if (!safe_parse(parts[3], ctype_val) || !safe_parse(parts[4], tempchecksum_value_) ||
            !safe_parse(parts[5], tempcontent_length_))
        return false;
    ...
    // 所有解析成功，提交更改（异常安全）
    headers_ = std::move(temp_headers);
    status_message_ = std::move(temp_status_message);
    server_ = std::move(temp_server);
    version_ = temp_version;
    status_code_ = temp_status_code;
    checksum_value_ = tempchecksum_value_;
    content_length_ = tempcontent_length_;
    checksum_type_ = tempchecksum_type_;
    timestamp_ = temp_timestamp;
